...
「ZIP に圧縮する」は選んだものをまとめて 1 つの書庫にします。「1 つずつ ZIP に圧縮する」は対象ごとに別々の書庫を作るため、フォルダをまとめて選べばフォルダの数だけ書庫が並びます。どちらも対象と同じ場所へ書き出します。「1 つずつ」の項目は、2 つ以上を選んだときだけ現れます。1 つでは普通の圧縮と変わらないためです。
