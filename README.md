...
| `GithubPush.bat` | バージョンを決め、ソースを push し、インストーラを Releases へ公開します |

`GithubPush.bat` は `scripts/release.mjs` を呼び出します（`npm run release` でも同じです）。次の順に進みます。

1. **バージョン** — いまの値を表示します。変えたい場合だけ入力してください。変えた場合はインストーラを作り直す必要がありますので、あとで `InstallerMake.bat` を実行してください。
...
3. **ソース** — コミットしていない変更があれば一覧で示し、メッセージを入力するとコミットします。そのうえで、送っていないコミットを `main` へ送ります。
4. **Releases** — インストーラに加えて `latest.yml` と `.blockmap` も送ります。`latest.yml` はアプリが更新を見つけるための目録で、これが無いと配布済みのアプリが新しい版に気づけません。
